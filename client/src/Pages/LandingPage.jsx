...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-blue-50 to-white text-gray-900 flex flex-col items-center">
      {/* Hero Section */}
      <section className="flex flex-col items-center justify-center text-center mt-20 px-4">
        <motion.h1
          ...
          className="text-5xl md:text-6xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 to-pink-500"
        >
      ...
      <section className="mt-24 grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-8 px-6 md:px-16 pb-20 w-full max-w-6xl">
        {products?.slice(0, 3).map((p) => (
          <motion.div
            ...
            className="backdrop-blur-xl bg-white/60 border border-gray-200 rounded-2xl p-5 shadow-lg hover:shadow-xl hover:shadow-indigo-100 transition"
          >
            <img
              src={p.image}
              alt={p.title}
            ...
            <p className="text-gray-600 mt-2 line-clamp-2">{p.desc}</p>
      ...
      <footer className="text-gray-500 pb-8 text-sm">
        © {new Date().getFullYear()} Stripe Imp. — Built with ❤️ by Abid Ullah
      </footer>
    </div>
  );
};
